...
setIsOpen(true)}
  ...
  try {
    ...
    doc.setFont("times", "normal");
    doc.setTextColor(0, 0, 0);
    doc.text(`Date: ${new Date(chatToExport.createdAt).toLocaleString()}`, 20, currentY);
    currentY += 10;
...
const toggleSelectAll = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`${REACT_APP_API_URL}/api/chats`).then((res) => {setChats(res.data);})
    .catch((err)=>{
      alert(err)
    })
    ;
 ...
 const handleVoiceInput = async () => {
  ...
  if (Capacitor.isNativePlatform()) {
    try {
      ...
      const partialResultsListener = SpeechRecognition.addListener("partialResults", (data) => {
        if (data.matches && data.matches.length > 0) {
          setInput(data.matches[0]);
        }
      });

      const listeningStateListener = SpeechRecognition.addListener("listeningState", (state) => {
