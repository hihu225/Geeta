...
const getCurrentUserId = async () => {
  try {
    // Try to get user ID from auth token stored via StorageService
...
const checkAndClearCacheForNewUser = async () => {
...
const saveTokenToBackend = async (token) => {
  ...
  if (currentFCMToken === token && !(await checkAndClearCacheForNewUser())) {
  ...
  try {
    ...
    const authToken = await StorageService.get("token");
    const userIdForRequest = await getCurrentUserId();

    const response = await axios.post(
      `${backend_url}/api/notifications/save-token`,
      {
        token: token,
        userId: userIdForRequest // Include user ID in request
      },
      {
    ...
    if (response.status === 200) {
      if (response.data.alreadyExists) {
        ...
        console.log("Token saved successfully to backend for user:", userIdForRequest);
      }
      // Cache the token after successful save
      currentFCMToken = token;
      currentUserId = userIdForRequest;
...
const FCMToken = async (navigate = null) => {
  ...
  if (Notification.permission === 'granted') {
    ...
    const userChanged = await checkAndClearCacheForNewUser();
...
// Kept synchronous — reads from the module-cached currentUserId populated by the async paths.
