...
class NotificationService {
  async sendDailyQuoteToUser(userId) {
    ...
    try {
      ...
      // Get quote from GROQ with user context for sequential quotes
      let quoteData = await groqService.getDailyQuote(
        user.preferences?.language || "english",
        user.preferences?.quoteType || "random",
        user // Pass user object for sequential progress tracking
      );
      //console.log("GROQ Quote Data:", quoteData);

      // If GROQ API fails, use fallback quote
      if (!quoteData.success) {
        console.warn("GROQ API failed, using fallback quote");
        quoteData = {
          success: true,
          quote:
      ...
      if (user.preferences?.quoteType === "sequential" && quoteData.success) {
        try {
          sequentialProgress = await groqService.advanceUserSequentialVerse(
            user
          );
          console.log(
      ...
      try {
        ...
        // Mark delivered and update lastSent only AFTER successful delivery
  ...
  shouldSendNotification(user) {
    try {
      ...
      // Only send if current time is AT or AFTER scheduled time (within 5-minute window)
